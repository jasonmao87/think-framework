...
let _debugEnable = true;

Array.prototype.pushMongoModal=function(filterMongoBeanModal){
    if(this.length == 0 ){
        _nDebug( filterMongoBeanModal,true);
        this.push( filterMongoBeanModal) ;
    }else{
        let index = 1;
        let key = filterMongoBeanModal.key;
        this.forEach(function (value) {
            if(value.key ==  filterMongoBeanModal.key ){
                filterMongoBeanModal.key ='#' + index + "##" + key ;
            }
            index ++ ;
        });
        this.push( filterMongoBeanModal)
...
function ThinkMongoFilter( ){
    this.filterBody = [];
    this._eq = function (k , v , dataType) {
        _nDebug("WebFilter 添加参数 "+ k + " == " + v + " type[ "+ dataType+"]") ;
        let modal = new filterMongoBeanModal(k,webSqlOp.opEq,v,dataType) ;
        this.filterBody.pushMongoModal(modal);
        return this;
    };
     ...
     * @param k
     * @param values
     * @returns {ThinkMongoFilter}
     */
    this.in = function(k , values ){
        if(values instanceof  Array){
            let modal = new filterMongoBeanModal(k,webSqlOp.opIn,values,webDataType.ARRAY);
            this.filterBody.pushMongoModal(modal);
        }else{
            _nWarn("操作没有执行！in方法的 values 参数必须是数组",true);
        }
        return this;
    ...
    //     if(values instanceof  Array){
    //         let modal = new filterMongoBeanModal(k,webSqlOp.opNotIn,values,webDataType.ARRAY);
    ...
    this.or = function(k ,values){
        if(values instanceof  Array){
            let modal = new filterMongoBeanModal(k,webSqlOp.opOr,values,webDataType.ARRAY);
            this.filterBody.pushMongoModal(modal);
        }else{
            _nWarn("操作没有执行！or方法的 values 参数必须是数组",true);
        }
        return this;
    };

    this.like = function(k ,likestr){
        let modal = new filterMongoBeanModal(k,webSqlOp.opLike,likestr,webDataType.STRING);
        this.filterBody.pushMongoModal(modal);
        return this;
    };
    this.lessThanDate = function(k,dateStr) {
        let modal = new filterMongoBeanModal(k,webSqlOp.opLessThan,dateStr,webDataType.DATE);
        this.filterBody.pushMongoModal(modal);
        return this;
    };
    this.lessThanNumber =  function(k,number) {
        let modal = new filterMongoBeanModal(k,webSqlOp.opLessThan,number,webDataType.NUMBER);
        this.filterBody.pushMongoModal(modal);
        return this;
    };
    ...
    this.lessThanAndEqNumber =  function (k,number) {
        let modal = new filterMongoBeanModal(k,webSqlOp.opLessAndEquals,number,webDataType.NUMBER);
        this.filterBody.pushMongoModal(modal);
        return this;
    };
    ...
    this.largeThanDate = function (k,dateStr) {
        let modal = new filterMongoBeanModal(k,webSqlOp.opLargeThan,dateStr,webDataType.DATE);
        this.filterBody.pushMongoModal(modal);
        return this;
    };
    this.largeThanNumber =  function (k,number) {
        let modal = new filterMongoBeanModal(k,webSqlOp.opLargeThan,number,webDataType.NUMBER);
        this.filterBody.pushMongoModal(modal);
        return this;
    };
    ...
    this.largeThanAndEqDate = function (k,dateStr) {
        let modal = new filterMongoBeanModal(k,webSqlOp.opLargeAndEquals,dateStr,webDataType.DATE);
        this.filterBody.pushMongoModal(modal);
        return this;
    };
    this.largeThanAndEqNumber =  function (k,number) {
        let modal = new filterMongoBeanModal(k,webSqlOp.opLargeAndEquals,number,webDataType.NUMBER);
        this.filterBody.pushMongoModal(modal);
        return this;
    };
    ...
    this.betweenAnd = function(k,v1,v2){
        let vk = [v1,v2];
        let modal = new filterMongoBeanModal(k,webSqlOp.opBetweenAnd,vk,webDataType.ARRAY);
        this.filterBody.pushMongoModal(modal);
        return this;
    }
    this.clear = function(){
     ...
     * @param _dataType
     */
    function filterMongoBeanModal(_key,_op,_value ,_dataType){
        if(isNaN(_value)){
            if(_value.indexOf('"')>-1){
        ...
        this.Value      = _value;
        this.type       =  _dataType;
        _nDebug("[MONGO]添加了新的参数：[值类型："+(this.type )+"]" );
        _nDebug("\t 关键字 :" + this.key + "" ,true);
        _nDebug("\t 操作符 :" + this.op + " " ,true);
        _nDebug("\t 值 :" + this.Value + " " ,true);
        _nDebug("\t 值数据类型 :" + this.type + " " ,true)
...
function _nDebug(info ,ig){
    if(_debugEnable){
        if(ig){
            console.info(  info)
        }else{
            console.info("来自ThinkMongoFilter的调试日志 :"  + info)
        }
    }
}

function _nWarn(info ,ig){
    if(_debugEnable){
        if(ig){
            console.warn(  info)
        }else{
            console.warn("来自ThinkMongoFilter的警告日志 :"  + info)
        }
    }
}
